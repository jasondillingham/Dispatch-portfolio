// Package poscan extracts the ERP reference numbers (POs first, items later)
...
// between. the ERP validation catches any false positives from the widened
// window.
...
// The result is low-confidence; callers must validate via the ERP lookup before
...
// no surrounding text to match). Accept looser matching — the ERP validation
...
// candidate via the the ERP PO lookup because 7-digit numbers are noisy.
