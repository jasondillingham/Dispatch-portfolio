// voucher.go — P21 voucher status sync. Background goroutine that polls
...
// runVoucherSync polls P21 for apinv_hdr status on every extraction that has
...
// drains over a few cycles without hammering P21.
//
...
// Clerks no longer set Done manually — the voucher posting in P21 *is* the
...
// pay_status anyway).
func runVoucherSync(p21c *p21.Client, cacheDB *cache.Cache, gc *graph.Client, mailbox string, interval time.Duration) {
	doVoucherSync(p21c, cacheDB, gc, mailbox, interval)
...
func doVoucherSync(p21c *p21.Client, cacheDB *cache.Cache, gc *graph.Client, mailbox string, interval time.Duration) {
	// Reconcile Done+unposted contradictions before the main sync runs.
	// These are rows where the Outlook tag says Status:Done but P21 reports
	...
	// P21 PO query per blocked row each tick.
	recheckBlocked(p21c, cacheDB, gc, mailbox)

	const batchSize = 50
	staleBefore := time.Now().Add(-interval).UTC()
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	cands, err := cacheDB.ListP21SyncCandidates(ctx, mailbox, staleBefore, batchSize)
	cancel()
	if err != nil {
	...
	for _, cand := range cands {
		if didAutoDone := syncOneVoucher(p21c, cacheDB, gc, mailbox, cand.MessageID, cand.PONo, cand.InvoiceNumber, &posted, &paid, &unposted, &errs); didAutoDone {
			autoDone++
		}
	}
...
func syncOneVoucher(p21c *p21.Client, cacheDB *cache.Cache, gc *graph.Client, mailbox, messageID string, poNo int64, invoiceNumber string, posted, paid, unposted, errs *int) bool {
	lookupCtx, lookupCancel := context.WithTimeout(context.Background(), 5*time.Second)
	ap, err := p21c.LookupAPInvoice(lookupCtx, poNo, invoiceNumber)
	lookupCancel()
	if err != nil {
		if errs != nil {
			*errs++
...
// ↓/↑) — gives them an "AP person enters voucher in P21, hits next, prior
...
func (s *server) handleRecheckVoucher(w http.ResponseWriter, r *http.Request) {
	...
	// Need PO + invoice number for the P21 lookup. Read from the cached
	...
	syncOneVoucher(s.p21, s.cache, s.gc, s.mailbox, msgID, ext.PONo, ext.Data.InvoiceNumber, nil, nil, nil, nil)
	w.WriteHeader(http.StatusNoContent)
}

// recheckBlocked re-runs reconciliation for every Status:Blocked message
// against fresh P21 PO lines. If the recon now reports clean (every line
...
func recheckBlocked(p21c *p21.Client, cacheDB *cache.Cache, gc *graph.Client, mailbox string) {
	if p21c == nil || gc == nil {
		return
	}
...
// P21 lookup came back unposted. Per-row work: GetCategories → drop Done →
...
func reconcileDoneUnposted(cacheDB *cache.Cache, gc *graph.Client, mailbox string) {
	...
	if fixed > 0 {
		log.Printf("done-unposted reconcile: cleared Status:Done on %d row(s) (P21 said unposted)", fixed)
	}
}
